...
try {
    $query = "SELECT username FROM `user_controllers` WHERE email = :email";
    ...
    if ($stmt->execute()) {
        ...
        if ($row) {
            $to = $email;
            $sub = 'Fandom PokePartners- Username Retrieval Request';

            $emailBody = "<html><body>\n \n";
            $emailBody .= "<p>Hello there,</p>\n \n";
            $emailBody .= "<p>Fandom PokePartners has received a request that a user is trying to retrieve their username. The username associated with this email is: <b>" . $row['username'] . "</b></p>\n \n";
